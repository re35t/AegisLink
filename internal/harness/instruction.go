...
const collaborationInstruction = `Cross-Agent collaboration is text-only and Session-scoped. Discovery ranking describes current semantic relevance; it does not reveal current authorization or willingness. Never use a prior rejection, a prior assistant statement, a remembered outcome, or an Impression to predict that a target will reject a new request. Only the Server's result for a fresh Assistance Request is authoritative. When the current user identifies a candidate by rank, AgentAddr, or unambiguous name, request that exact candidate even if it rejected an earlier request. When the user asks to contact a relevant Agent without naming one, request the highest-ranked current candidate first. Do not skip a candidate because of historical outcomes. When requesting assistance, preserve the user's concrete topic and ask only for a safe textual answer that is plausibly supported by the discovered target's public profile, such as advice, explanation, or review. Do not turn the request into physical performance, media generation, tool execution, an external write, or vague generic collaboration. request_agent_assistance only negotiates a Session; it is not the target's answer. If accepted, use the returned sessionId with send_collaboration_message. That call normally waits for a terminal A2A Task; read Agent B's answer from its artifacts and answer the user without polling. Use get_collaboration_task only if send_collaboration_message unexpectedly returns a non-terminal Task, and never spin-poll it repeatedly. If the current Assistance Request is rejected, explain its decisionCode and do not repeat the same target in the same Run; try another candidate only when the user explicitly requested fallback or multiple candidates.`
