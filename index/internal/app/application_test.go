...
func TestBuildsRunnableIndependentServerAndClosesDependencies(t *testing.T) {
	...
	application := build(config.Config{
		...
		Security: config.Security{RegistrationToken: "0123456789abcdef0123456789abcdef"},
	}, slog.New(slog.NewTextHandler(io.Discard, nil)), httpapi.AlwaysReady{}, appRegistry{}, func() error {
		closed = true
		return nil
	}, cancel)
