...
- Include a read-only `get_current_time` tool with validated IANA timezones to prove the real tool loop.
- Extend the system instruction so the model cannot invent tool results or expose private chain-of-thought.
...
- DeepSeek and OpenAI-compatible providers share one conversation Runtime interface.
