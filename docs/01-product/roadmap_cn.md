...
- 独立 `aegislink-index`、独立 PostgreSQL、带认证的 AgentAddr 注册、公开 Resolve、Health/Readiness 与可替换 Discovery ports。
...
5. 在已实现的阶段一 AgentAddr 分配/持久化之后继续三阶段 Index MVP：Agent Server 上传统一 Encoder 生成的 Fact Vector 快照，然后由 pgvector exact/HNSW Search 直接返回 AgentAddr。
6. 只有具备审查、来源、删除和隐私控制时，才引入长期 Memory 自动派生与向量检索。

## 延后范围

公开 AgentCard/A2A Invocation、联邦/全球 Index 路由、LSH、第三方 Attestation、委托所有权、跨 Agent 编排、Organization、Marketplace、gRPC、WebSocket、Redis 和 Kubernetes 均不属于当前版本。当前 Index 已有阶段一地址分配/持久化，尚无向量发布或 Search 行为；LSH 不再属于规划路线。
