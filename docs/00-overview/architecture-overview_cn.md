...
AegisLink 是 Go 模块化单体与 React Web 客户端组成的 Personal Agent OS。其主要部署单元是一个 API Server、一个 Web Client 和一个 PostgreSQL 数据库。仓库还包含可独立运行、使用自己 PostgreSQL AgentAddr Registry 的 Agent Index；这段第一跳导航能力尚未让产品成为可工作的 Multi-Agent Network。
...
Agent Server --未来 Routing Projection--> aegislink-index
                                                 └── 当前已有 AgentAddr Registry
```
...
| `index`                 | 独立 Index 进程、AgentAddr Registry Contract/Storage 与 Discovery ports |
