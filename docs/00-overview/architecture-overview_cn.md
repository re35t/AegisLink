...
Gin（internal/httpapi）
            │
领域 Service / Conversation Orchestrator / Context Resolver
            │
  ┌─────────┴──────────┐
...
flowchart LR
    ...
    chat["Conversation"] -->|"AG-UI"| run["Run 编排"] --> runtime["Eino Runtime"]
    run -->|"成功后同事务入队"| job["Curator Job"] --> curator["无 Tool Curator"] --> cognition["Impression / Fact Candidate"]
...
- **执行链**：AG-UI 只承载活动 Run；`conversation` 从 PostgreSQL 重新解析 Agent、历史和能力，再调用 Runtime。
...
- 业务模块（`account`、`agent`、`conversation`、`agentcontext`、`memory`、`impression`、`discovery`、`a2a`、`skills`、`mcp`、`catalog`）只使用 `context.Context`、领域类型和小接口。
- `internal/conversation` 负责 Run 编排、取消、持久化事件、能力选择和 Runtime 调用。
- `internal/runtime` 封装 Eino 与模型供应商 SDK，其他模块只依赖 `conversation.Runtime`。
...
| `internal/conversation` | Run 创建、取消、事件持久化、Runtime 调用                            |
| `internal/agentcontext` | 聚合 Memory、Skill、MCP、Confirmed Fact 与排序后的 Impression       |
| `internal/runtime`      | Eino、Model Provider、动态 Tool 和独立 Curator 模型调用             |
