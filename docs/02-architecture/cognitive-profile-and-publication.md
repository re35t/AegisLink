...
A standalone Index implements AgentAddr allocation, complete Fact Vector snapshot replacement, and pgvector exact Search with no public resolve route. Automatic Agent Server publication still needs to derive and upload vectors only from `public + indexable` AgentFacts; Index stores no Facts URL and never fetches AgentFacts. `aegislink.agent-facts/1.0-draft` remains an AegisLink-owned, versioned, Server-authoritative document rather than a claim of an external standard.
